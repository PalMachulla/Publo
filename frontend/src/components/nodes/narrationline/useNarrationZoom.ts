...
export function useNarrationZoom({ 
  ...
  const fitToView = useCallback(() => {
    ...
    // Available width = viewport - sticky label (64px) - left padding (8px) - buffer (56px for proper display)
    const stickyLabelWidth = 64
    const leftPadding = 8   // Left padding from pl-2
    const buffer = 56       // Right buffer to prevent clipping - matches content area buffer
